...
maxcamber = 5;    // percent figure. ( valid in range 0-9 )

camberposn = 4; // tenths   ( valid in range 0-9 )

thickness = 20; //percentage relative to chord   ( valid in range 0-99 )
...
module cylinder(s_radius, length) {
		rotate([0, 180, 0]) rotate_extrude($fn=100) translate([s_radius, 0, 0]) {
			rotate([0, 0, 90]) {
				scale([0.4, 0.01, 1]) circle(10);
			}
		}
}
module circular_wing(s_radius, length) {
		rotate([0, 180, 0]) rotate_extrude($fn=100) translate([s_radius, 0, 0]) {
			rotate([0, 0, 90]) {
				scale([length, length, 1]) airfoil();
			}
		}
}
cylinder(4,1);
//translate([-2,0,0]) scale([2,2,0]) airfoil();
//translate([-2,3,0]) square([4,0.5]);
//translate([-2,-3,0]) scale([4,2,0]) airfoil();
//circular_wing(3.5, 4);